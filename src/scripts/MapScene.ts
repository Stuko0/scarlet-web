...
// @ts-ignore - simplex-noise import
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  (async function () {
    try {
      ...
      const hexes: Record<string, Array<{height: number, position: Vector2}>> = { stone: [], dirt: [], dirt2: [], sand: [], grass: [], fire: [] };
      const extras: Record<string, Array<{height: number, position: Vector2}>> = { trees: [], fires: [], stones: [] };
      ...
      baseHexGeo.translate(0, 0.5, 0);

      const dummy = new Object3D();

      function createInstancedHexes(data: Array<{height: number, position: Vector2}>, mapTexture: any) {
      ...
      hexMeshes.forEach(m => scene.add(m));
      ...
      if (extras.fires.length > 0) {
        const geo1 = new CylinderGeometry(0, 1, 1, 4); geo1.translate(0, 0.5, 0); geo1.rotateY(Math.PI / 4);
        const geo2 = new CylinderGeometry(0, 0.8, 1, 4); geo2.translate(0, 1.1, 0); geo2.rotateY(Math.PI / 8);
        const geo3 = new CylinderGeometry(0, 0.6, 1, 3); geo3.translate(0, 1.7, 0);
        ...
        extras.fires.forEach((item, i) => {
          dummy.position.set(item.position.x, item.height + 0.8, item.position.y);
          const scaleX = Math.random() * 0.4 + 0.6;
          const scaleY = scaleX * (Math.random() * 0.5 + 1);
          dummy.scale.set(scaleX, scaleY, scaleX);
          dummy.rotation.set(Math.random() * 0.15, Math.random() * Math.PI, Math.random() * 0.15);
          dummy.updateMatrix();
          mesh.setMatrixAt(i, dummy.matrix);
        });
        scene.add(mesh);
      ...
      let seaMesh = new Mesh(
        ...
        new MeshPhysicalMaterial({
          ...
          ior: 1.4, transmission: 1, transparent: true,
      ...
      // Signal that the 3D model is ready — TreeLoader listens for this
      window.dispatchEvent(new Event('modelLoaded'));
    } catch (error) {
      console.error('Failed to load 3D scene:', error);
      // Signal failure so the loader can show an error state
      window.dispatchEvent(new CustomEvent('modelError', { detail: error }));
    }
  })();
